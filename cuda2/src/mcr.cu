...
//#define screen_width 512    // screenwidth
...
int main (int argc, char* argv[]) {
    ...
    vec3 * host_output = new vec3[screen_width * screen_height];

    // Pointer to the image on the device (GPU)
    vec3 * device_output;

    // Allocate memory on CUDA device
    cudaMalloc(&device_output, screen_width * screen_height * sizeof(vec3));
    ...
    cudaMalloc((void **)&device_rand_state,  screen_width * screen_height *sizeof(curandState));
...
void render_kernel(
    ...
    vec4 dir(((float)x - screen_width / 2) , ((float)y - screen_height / 2) , focal_length , 1); 
