...
def preProcessDepth():
    with open("data/full/noisy_depth.txt") as f:
        ...
        gradx = np.reshape(gradx, (config.IMAGE_HEIGHT, config.IMAGE_WIDTH, 1))
        grady = np.reshape(grady, (config.IMAGE_HEIGHT, config.IMAGE_WIDTH, 1))
...
full_images = {
    ...
    "sn_grady" : sn_gradx,
    "sn_var" : sn_var,
    "albedo_gradx" : albedo_gradx,
    "albedo_grady" : albedo_gradx,
    "albedo_var" : albedo_var,
    "depth_gradx" : depth_gradx,
    "depth_grady" : depth_gradx,
    "depth_var" : depth_var
}
