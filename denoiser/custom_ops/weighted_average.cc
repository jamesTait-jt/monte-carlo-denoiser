...
class WeightedAverageOp : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext * context) override {
            ...
            DCHECK_EQ(weights_shape.dim_size(2) % 2, 1);

            // Get the image dimensions
            const int padded_img_width = img_shape_with_padding.dim_size(0);
            const int padded_img_height = img_shape_with_padding.dim_size(1);

            // Ensure the image is square
            DCHECK_EQ(padded_img_width, padded_img_height);

            // Ensure the kernel is smaller than the padded image
            DCHECK(weights_shape.dim_size(2) < padded_img_width);

            // Get the radius of the kernel
            const int kernel_radius = std::floor(weights_shape.dim_size(2) / 2);

            // Get the accessible version of the noisy image tensor
            auto noisy_tensor = noisy_img.tensor<float, 3>();

            // Get the accessible version of the output tensor
            auto prediction_tensor = prediction_img->tensor<float, 3>();
                ...
                for (int j = 0 ; j < padded_img_width ; j++) {
                    ...
                    if ((i < kernel_radius) || (i + kernel_radius >= padded_img_height) ||
                       (j < kernel_radius) || (j + kernel_radius >= padded_img_width)) {
                        prediction_tensor(i, j, 0) = 0;
                        prediction_tensor(i, j, 1) = 0;
                        prediction_tensor(i, j, 2) = 0;
                    ...
                    // neighbourhood
                        ...
                        for (int k1 = -kernel_radius ; k1 <= kernel_radius ; k1++) {
                            for (int k2 = -kernel_radius ; k2 <= kernel_radius ; k2++) {
                                float weight_value = weights_tensor(i, j, k1, k2);
                                float neighbour_value_r = noisy_tensor(k1 + i, k2 + j, 0);
                                float neighbour_value_g = noisy_tensor(k1 + i, k2 + j, 1);
                                float neighbour_value_b = noisy_tensor(k1 + i, k2 + j, 2);
                                sum_r += neighbour_value_r * weight_value;
                                sum_g += neighbour_value_g * weight_value;
                                sum_b += neighbour_value_b * weight_value;
                            }
                        }
                        prediction_tensor(i, j, 0) = sum_r;
                        prediction_tensor(i, j, 1) = sum_g;
                        prediction_tensor(i, j, 2) = sum_b;
                    }
                }
            }
